extern crate intelflash;
extern crate plain;
extern crate uefi;

use intelflash::{Rom, BiosFile, BiosSection, BiosSections, BiosVolume, BiosVolumes};
use intelflash::{section, volume};
...
fn intelflash(path: &str) -> io::Result<()> {
    println!("{}", path);
...
fn main() {
    for arg in env::args().skip(1) {
        if let Err(err) = intelflash(&arg) {
            eprintln!("intelflash: {}: {}", arg, err);
            process::exit(1);
        }
    }
}
